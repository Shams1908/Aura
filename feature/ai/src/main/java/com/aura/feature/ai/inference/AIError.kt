...
sealed class AIError(val msg: String, cause: Throwable? = null) : Exception(msg, cause) {
    ...
    class ModelUnavailable(val category: String? = null, val confidence: Float? = null) : AIError("MODEL_UNAVAILABLE: Real on-device fashion segmentation model is not bundled in assets.")
    ...
    class SegmentationFailed(val category: String? = null, val confidence: Float? = null) : AIError("Segmentation failed: No clothing mask pixels resolved within detection region.")
}
